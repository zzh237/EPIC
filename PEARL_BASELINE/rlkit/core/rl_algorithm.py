...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.sampler = InPlacePathSampler(
            env=env,
            policy=agent,
            max_path_length=self.max_path_length,# self.max_path_length == H in out paper for_zhangzhi
        )
        ...
        self._M = M # M is the number of episodes we can collect from each task
        #for_zhangzhi N==self.train_tasks
        # [buffer_0, buffer_1, ... ,buffer_N]
    ...
    def collect_data(self, num_samples, resample_z_rate, update_posterior_rate, add_to_enc_buffer=True):
        ...
        while num_transitions < num_samples:
            paths, n_samples, n_trajs = self.sampler.obtain_samples(max_samples=np.inf,
        ...
        self._n_env_trajs_total += n_trajs
        # for_zhangzhi
        # I add something hear to log rewards during training
        average_reward = 0
        for path in paths:
            average_reward = average_reward + np.sum(path['rewards'])
        average_reward = average_reward/len(paths)
        gt.stamp('sample')
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        while num_transitions < self.num_steps_per_eval:
            path, num, trajs = self.sampler.obtain_samples(deterministic=self.eval_deterministic, max_samples=self.num_steps_per_eval - num_transitions, max_trajs=1, accum_context=True)
            paths += path
            num_transitions += num
            num_trajs += trajs
    ...
    def evaluate(self, epoch):
        ...
        #         self.agent.infer_posterior(context)
        #         p, _, _ = self.sampler.obtain_samples(deterministic=self.eval_deterministic, max_samples=self.max_path_length,
        #                                                 accum_context=False,
        #                                                 max_trajs=1,
        #                                                 resample=np.inf)
        #         paths += p
