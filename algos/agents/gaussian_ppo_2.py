...
class GaussianPPO2(EPICModel):
    """
    A meta policy (maintaining a gaussian distribution of policies)
    """
    def __init__(self, state_space, action_space, meta_update_every, hidden_sizes=(4,4), 
        ...
        if isinstance(action_space, Discrete):
            ...
            self.policy_hub = PolicyHub(state_dim, self.action_dim, hidden_sizes, activation, "GaussianPPO", tau, self.device)
    ...
    def act_m(self, m: int, state) -> Action:
        state, action, log_prob = self.cur_policy.act(state, self.device)
        return Action(
            state=state,
            action=action,
            log_prob=log_prob
    ...
    def per_step_m(self, m:int, meta_episode, step, action_dict: Action, reward, new_state, done):
        ...
        self.memory.add(state.detach(), action.detach(), log_prob.detach(), reward, done)
    ...
    def update_prior(self) -> None:
        self.meta_update(self.memory)
    
    def update_default(self) -> None:
        # TODO don't do anything?
        pass
    
    def pre_meta_episode(self) -> None:
        # draw a policy from the prior
        self.sample_policy()
    ...
    #     print("build a copy")
    #     temp_hub = PolicyHub(state_dim=self.policy_hub.state_dim, 
    #                          action_dim=self.policy_hub.action_dim,
    #         hidden_sizes=self.policy_hub.hidden_sizes, 
    #         activation=self.policy_hub.activation, 
    #         algo="GaussianPPO",
    #         tau=self.policy_hub.tau, 
    #         device=self.device)
