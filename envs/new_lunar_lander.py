...
class NewLunarLander(gym.Env, EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        if  abs(state[0]) >= 1.0:
            ...
            reward = -100
        if self.game_over or not self.lander.awake:
            done = True
            reward = 100 * math.exp(-abs(state[0]-self.goal))
...
class LunarLanderContinuous(NewLunarLander):
    continuous = True
...
if __name__ == '__main__':
    demo_heuristic_lander(NewLunarLander(), render=True)